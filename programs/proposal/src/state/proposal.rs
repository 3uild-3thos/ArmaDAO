use crate::{constants::*, errors::ProposalError};
...
impl Proposal {
    ...
    pub fn init(
        ...
        require!(name.len() < 33, ProposalError::InvalidName);
        require!(metadata.len() < 113, ProposalError::InvalidGist);
        self.id = id;
        self.name = name;
    ...
    pub fn check_expiry(&self) -> Result<()> {
        require!(Clock::get()?.slot < self.expiry, ProposalError::Expired);
        Ok(())
    }
    pub fn is_analysed(&self) -> Result<()> {
        require!(
            Clock::get()?.slot >= self.created_time + self.evaluation_period,
            ProposalError::InvalidRequiredTime
        );
        Ok(())
    }
    ...
    pub fn is_failed(&self) -> Result<()> {
        require!(
            self.result == ProposalStatus::Failed,
            ProposalError::InvalidProposalStatus
        );
        Ok(())
    }
    ...
    pub fn is_open_and_expiry(&self) -> Result<()> {
        if self.is_open().is_ok() && self.check_expiry().is_ok() {
            return err!(ProposalError::InvalidProposalStatus);
        }
        Ok(())
    }
