...
impl StakeState {
    ...
    pub fn unstake(
        ...
        self.amount = self.amount.checked_sub(amount).ok_or(StakeError::Underflow)?;
        self.update()
    }
    ...
    pub fn check_slot(&self) -> Result<()> {
        require!(self.updated < Clock::get()?.slot, StakeError::InvalidSlot);
        Ok(())
    }    

    // Make sure the user doesn't have any open accounts
    pub fn check_accounts(&self) -> Result<()> {
        require!(self.accounts == 0, StakeError::AccountsOpen);
        Ok(())
    }
    ...
    pub fn check_locked_amount(&self, amount: u64) -> Result<()> {
        require!(amount <= self.amount - self.locked_amount, StakeError::LockedToken);
        Ok(())
    }
    ...
    pub fn check_stake(&self) -> Result<()> {
        require!(self.amount > 0, StakeError::InsufficientStake);
        Ok(())
    }

    // Ensure staked amount > X
    pub fn check_stake_amount(&self, amount: u64) -> Result<()> {
        require!(self.amount >= amount, StakeError::InsufficientStake);
        Ok(())
    }
}  